...
vector<uint8_t> ntest::extract_binary_file(string const &path)
{
  ...
  source_location const loc)
{
  ...
  {
    if (expected_exists)
      return extract_binary_file(expected_path_generic);
    else
  ...
  {
    if (actual_exists)
      return extract_binary_file(actual_path_generic);
    else
      return vector<uint8_t>();
  }();
